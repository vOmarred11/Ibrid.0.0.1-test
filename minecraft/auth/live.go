...
func tokenExists(filename string) bool {
	_, err := os.Stat(filename)
	// Se err è nil, il file esiste, altrimenti non esiste
...
func RequestLiveTokenWriter(w io.Writer) (*oauth2.Token, error) {
	...
	for range ticker.C {
		...
		if t != nil {
			...
			fmt.Println("Local Connection started, join using your ipv4 or 127.0.0.1")
			return t, nil
		}
	}
	panic("unreachable")
