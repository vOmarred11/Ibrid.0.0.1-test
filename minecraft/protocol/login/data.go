...
func (data IdentityData) Validate() error {
	...
	if strings.Contains(data.DisplayName, "ur-name") {
		fmt.Println("[NetworkSession]->Loading packets from:", data.DisplayName)
	} else {
		return fmt.Errorf("[NetworkSession]-> invalid packets from", data.DisplayName)
	}
	return nil
}
